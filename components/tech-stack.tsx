...
const imageTech = (src: string, label: string, href: string): LogoItem => ({
  node: (
    <span className="fnlb-logo-wrap relative inline-flex h-8 w-8 items-center justify-center overflow-hidden" aria-label={label}>
      <img
        src={src}
        alt={label}
        className="fnlb-logo h-8 w-8 scale-[3.2] object-contain transition-all duration-300 ease-out"
        draggable={false}
      />
      <img
        src={src}
        alt=""
        aria-hidden="true"
        className="fnlb-logo-hover pointer-events-none absolute h-8 w-8 scale-[3.4] object-contain opacity-0 transition-opacity duration-300 ease-out"
        draggable={false}
      />
    </span>
  ),
