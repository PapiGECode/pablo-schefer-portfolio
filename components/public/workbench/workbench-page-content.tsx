...
const wipItems = [
  ...
  },
  {
    id: 5,
    name: "PapiGECode",
    description: "Professional GitHub profile README and developer identity",
    progress: 55,
    lastUpdated: "Today",
...
const recentActivity = [
  { type: "commit", project: "pablo-schefer-portfolio", message: "Refine professional developer identity", time: "today" },
  { type: "branch", project: "duolingo-streak-keeper", message: "Improve the automation workflow", time: "today" },
  { type: "commit", project: "KiCord-DOOM-Plugin", message: "Extend the KiCord plugin", time: "this week" },
  { type: "commit", project: "PapiGECode", message: "Update developer profile README", time: "this week" },
]
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className={cn("mb-12 sm:mb-16 space-y-4 opacity-0", isVisible && "animate-fade-in-up")}>
          <p className="font-mono text-xs uppercase tracking-[0.25em] sm:tracking-[0.35em] text-primary">
            <AnimatedText text={language === "es" ? "Trabajo en curso" : "Work in Progress"} emphasis="label" />
          </p>
          <h1 className="text-4xl font-bold tracking-tight sm:text-5xl lg:text-6xl"><AnimatedText text={language === "es" ? "Mesa de trabajo" : "Workbench"} mode="chars" emphasis="title" /></h1>
          <p className="max-w-2xl text-base sm:text-lg text-muted-foreground leading-relaxed">
            <AnimatedText text={language === "es" ? "Experimentos y prototipos activos. Cosas que se construyen, se rompen y se vuelven a construir. Progreso en tiempo real de proyectos en curso." : "Active experiments and prototypes. Things that are being built, broken, and rebuilt. Real-time progress on ongoing projects."} />
          </p>
        </div>
        ...
        <div className="grid gap-8 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2">
            ...
            >
              ...
              <div className="flex items-center gap-3 border-b border-border/50 bg-secondary/40 px-4 sm:px-5 py-3.5 sm:py-4">
                ...
                <span className="ml-4 font-mono text-xs text-muted-foreground truncate">~/pabloschefer/active</span>
                <div className="ml-auto flex items-center gap-2 text-muted-foreground">
                  <span className="h-1.5 w-1.5 rounded-full bg-primary animate-pulse" />
                  <span className="font-mono text-xs">{language === "es" ? "en directo" : "live"}</span>
                </div>
              </div>
                ...
                {wipItems.map((item, index) => (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between gap-6 pl-6 sm:pl-0 sm:justify-end">
                      ...
                      <span className="font-mono text-xs text-muted-foreground shrink-0">{language === "es" ? "Hoy" : item.lastUpdated}</span>
                    </div>
                  </a>
                ))}
              </div>
              ...
              <div className="border-t border-border/50 bg-secondary/30 px-4 sm:px-5 py-4">
                <div className="flex items-center gap-2 font-mono text-xs text-muted-foreground">
                  ...
                  <span className="ml-auto text-primary/50 hidden sm:block">{language === "es" ? "pulsa enter para ejecutar" : "press enter to run"}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            >
              <h3 className="font-mono text-xs uppercase tracking-wider text-primary mb-4">{language === "es" ? "Estadísticas" : "Stats"}</h3>
              <div className="grid grid-cols-2 gap-4">
                <div className="text-center p-3 rounded-lg bg-secondary/30">
                  <p className="text-2xl font-bold text-foreground">{wipItems.length}</p>
                  <p className="font-mono text-xs text-muted-foreground">{language === "es" ? "Activos" : "Active"}</p>
                </div>
                <div className="text-center p-3 rounded-lg bg-secondary/30">
                  <p className="text-2xl font-bold text-primary">
                    {Math.round(wipItems.reduce((a, b) => a + b.progress, 0) / wipItems.length)}%
                  </p>
                  <p className="font-mono text-xs text-muted-foreground">{language === "es" ? "Progreso medio" : "Avg Progress"}</p>
                </div>
              </div>
            </div>
                  ...
                  <div key={index} className="flex items-start gap-3 text-xs">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-muted-foreground flex items-center gap-1">
                        ...
                        {language === "es" ? (activity.time === "today" ? "hoy" : "esta semana") : activity.time}
                      </p>
                    </div>
                  </div>
                ))}
